...
def plot(data, net):
    ...
    antide_true = np.sin(2 * np.pi * x) / (2 * np.pi)
    antide_pred = net.predict(cos[:, None], returnnp=True).squeeze()
    
    plt.plot(x, cos, color='black', label=r'Input: $\cos(2\pi x)$', zorder=0)
    plt.plot(x, antide_true, color='b', label=r'Output: $\sin(2\pi x)/(2\pi)$', zorder=1)
    plt.plot(x, antide_pred, color='r', label=r'Prediction', zorder=2)
    plt.legend()

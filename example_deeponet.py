...
def plot(data, net):
    ...
    antide_pred = net.predict(np.hstack([np.tile(cos, (len(y), 1)), y[:, None]]), returnnp=True).squeeze()
...
def main():
    ...
    net = ln.nn.DeepONet(data.branch_dim, data.trunk_dim, branch_depth, trunk_depth, width, activation)
    args = {
        'data': data,
        'net': net,
        'criterion': 'MSE',
